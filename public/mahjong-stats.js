...
import {n as i, e as r} from './select-option-ec247a15.js';
...
let c = class extends s {
  ...
  async _loadData() {
    const t = (await d(n(l, 'results'))).docs,
      ...
      s = [],
      a = [],
      i = this._gameType.value || '四麻',
      r = Number(this._targetYear.value) || new Date().getFullYear();
    t.forEach((t) => {
      if (
        t.data().gameInfo.gameType !== i ||
        new Date(t.data().gameInfo.date).getFullYear() !== r
      )
        return;
      const o = t.data().results;
      e.push(...o);
      const d = t.data().chonbo;
      d?.length > 0 && s.push(...d);
      const n = t.data().yakuman;
      if (n?.length > 0) {
        const e = n.map((e) => ({...e, date: t.data().gameInfo.date}));
        a.push(...e);
      }
    }),
...
(c.styles = [
  ...
  h([i({type: Array})], c.prototype, 'yakumanList', void 0),
  h([r('#gameType')], c.prototype, '_gameType', void 0),
  h([r('#targetYear')], c.prototype, '_targetYear', void 0),
